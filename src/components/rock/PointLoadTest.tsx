...
const PointLoadTest = () => {
  ...
  // Is(50) = P / De² (simplified point load index)
  const getIndex = (row: Row) => {
    const p = parseFloat(row.failureLoad);
    const de = parseFloat(row.de);
    if (!p || !de) return "";
    return ((p * 1000) / (de * de)).toFixed(3); // MPa
  };
  ...
  return (
    <TestSection title="Point Load Test" onSave={() => {}} onClear={() => setRows([{ sampleId: "", failureLoad: "", de: "" }])}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b">
              <th className="text-left py-2 px-2 font-medium text-muted-foreground">Sample ID</th>
              <th className="text-left py-2 px-2 font-medium text-muted-foreground">Failure Load (kN)</th>
              <th className="text-left py-2 px-2 font-medium text-muted-foreground">De (mm)</th>
              <th className="text-left py-2 px-2 font-medium text-muted-foreground">Is(50) (MPa)</th>
      ...
      <Button variant="outline" size="sm" className="mt-3" onClick={() => setRows([...rows, { sampleId: `P${rows.length + 1}`, failureLoad: "", de: "" }])}>
