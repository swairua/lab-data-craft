...
describe("Atterberg JSON export/import", () => {
  it("preserves the current export payload shape", () => {
          ...
          tests: [
            {
              ...
              type: "liquidLimit",
              isExpanded: true,
              trials: [{ id: "trial-1", trialNo: "1", penetration: "20", moisture: "30" }],
    ...
    const firstTest = state?.records[0].tests[0];
    expect(firstTest?.type === "liquidLimit" && firstTest.trials[0].moisture).toBe("30");
  });
  ...
  it("normalizes legacy payloads", () => {
    ...
    const firstTest = state?.records[0].tests[0];
    expect(firstTest?.type === "plasticLimit" && firstTest.trials[0].moisture).toBe("22");
  });
...
describe("Atterberg JSON normalization functions", () => {
  describe("normalizeAtterbergProjectState - advanced scenarios", () => {
          ...
          records: [
            {
              ...
              tests: [
                {
                  type: "liquidLimit",
                  trials: [{ penetration: "20", moisture: "30" }],
                },
              ],
            },
          ],
            ...
            tests: [
              {
                ...
                testTitle: "Legacy SL",
                shrinkageLimitRows: [
                  { trialNo: "1", initialVolume: "140", finalVolume: "130", moisture: "12" },
                ],
    ...
    it("creates default trial when none provided", () => {
      ...
      expect(state?.records[0].tests[0].trials).toHaveLength(1);
      const trial = state?.records[0].tests[0].trials[0] as any;
      expect(trial.penetration).toBe("");
      expect(trial.moisture).toBe("");
    });
            ...
            tests: [
              {
                type: "liquidLimit",
                isExpanded: true,
                trials: [{ penetration: "20", moisture: "28" }],
              },
            ],
          },
        ],
